...
def search_live_bin(token: str, marketplace_id: str, q: str, limit: int, category_id: str = "", max_price_gbp: float = 0.0) -> List[Dict[str, Any]]:
    ...
    params["sort"] = "price"  # cheapest-first
...
def run_scan(
    ...
    # Step 1: Collect console_ids used by selected profiles
    console_ids_needed = set()
    for prof_name in selected_profiles:
        p = profiles.get(prof_name, {})
        cid = p.get("console_id")
        if cid in consoles:
            console_ids_needed.add(cid)

    # Step 2: Fetch once per (console_id, base) with API-side price cap
    items_by_console: Dict[str, List[Dict[str, Any]]] = {}
    if not offline_mode:
        for console_id in console_ids_needed:
            ...
            for b in bases:
                if not b:
                    continue
                if scan_mode == "bin":
                    console_items += search_live_bin(token, marketplace, b, FETCH_LIMIT, max_price_gbp=cap)
                elif scan_mode == "auctions_ending":
                    console_items += search_live_auctions_ending(token, marketplace, b, FETCH_LIMIT, ending_hours, max_price_gbp=cap)
                else:
                    console_items += search_live_both(token, marketplace, b, FETCH_LIMIT, ending_hours, max_price_gbp=cap)
            items_by_console[console_id] = console_items

    # Step 3: Loop profiles, filter from cached items
    for prof_name in selected_profiles:
        p = profiles.get(prof_name, {})
        ...
        # Apply fault_q as local title filter (all words must appear in title)
        if fault_q and not df_live.empty and "title" in df_live.columns:
            words = [re.escape(w.lower()) for w in fault_q.split() if w.strip()]
            if words:
                t = df_live["title"].fillna("").str.lower()
                for w in words:
                    t_mask = t.str.contains(w, regex=True, na=False)
                    df_live = df_live[t_mask]
                    if df_live.empty:
                        break
                    t = t[t_mask.index.isin(df_live.index)]
